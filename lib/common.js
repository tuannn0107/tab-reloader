const lastExcutionClearCookieKey = 'auto-clear-cookie@example.com_last_execution';
...
/*function reload(tabId, obj) {
    ...
    if (obj.form) {
        ...
        var executing = chrome.tabs.executeScript(tabId, {
           file: '/lib/auto-click-link.js'
        });
...
function reload(tabId) {
    let lastExcutionClearCookie = (((new Date()).getTime()) - ((new Date()).getTime())%(20 * 60 * 1000));
    let diff =  ((new Date()).getTime() - lastExcutionClearCookie);
    let vperiod = storage[tabId].vperiod;

    if (vperiod && (diff < (vperiod * 1.5))) {
            executeScriptOganicSearch(tabId);
            window.setTimeout(function() {
                executeScriptClickOrganicSearch(tabId)
            }, 3000);
            return;
    }

    executeScriptNavigateInternal(tabId);
}


function executeScriptClickOrganicSearch(tabId) {
...
function executeScriptOganicSearch(tabId) {
    chrome.tabs.executeScript(tabId, {
        file: '/lib/auto-click-link-oganic-search.js',
        allFrames: true
    });
}


function executeScriptNavigateInternal(tabId) {
    chrome.tabs.executeScript(tabId, {
        file: '/lib/auto-click-link-internal.js',
        allFrames: true
    });
}
